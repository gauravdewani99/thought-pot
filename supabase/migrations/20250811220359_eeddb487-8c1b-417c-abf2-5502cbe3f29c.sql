...
-- Use existing update function if present, else fallback
-- Prefer using the already provided update_updated_at_column if it exists
-- Attempt to create trigger with the known function name
...
create policy if not exists "Users can view their own note_chunks"
  on public.note_chunks for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own note_chunks"
  on public.note_chunks for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own note_chunks"
  on public.note_chunks for update
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own note_chunks"
...
create policy if not exists "Users can view their own chat_messages"
  on public.chat_messages for select
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own chat_messages"
  on public.chat_messages for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own chat_messages"
  on public.chat_messages for update
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own chat_messages"
...
create policy if not exists "Users can read their own note files"
  on storage.objects for select
  using (
    bucket_id = 'notes-uploads' and auth.uid()::text = (storage.foldername(name))[1]
  );

create policy if not exists "Users can upload their own note files"
  on storage.objects for insert
  with check (
    bucket_id = 'notes-uploads' and auth.uid()::text = (storage.foldername(name))[1]
  );

create policy if not exists "Users can update their own note files"
  on storage.objects for update
  using (
    bucket_id = 'notes-uploads' and auth.uid()::text = (storage.foldername(name))[1]
  );

create policy if not exists "Users can delete their own note files"
